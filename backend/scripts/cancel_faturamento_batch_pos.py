...
def cancel_faturamento_batch_pos():
    ...
    try:
        ...
        db = SessionLocal()
        target_pos = [
            '214088', '213953', '213940', '213911', '213822',
            '213688', '213657', '213537', '213518', '213421',
            '213354', '213260', '213089'
        ]
        ...
        print(f"Found {len(pos)} of {len(target_pos)} POs to cancel: {[p.po_number for p in pos]}")
        now_utc = datetime.utcnow()
        details_text = "Cancelamento solicitado via e-mail pelo Faturamento (NF cancelada no ONET) em 17/09/2026"
        ...
        for po in pos:
            ...
            # Update status for each order item and create formal audit log
                ...
                for item in po.items:
                    ...
                    audit_entry = AuditLog(
                        ...
                        extra_data={
                            ...
                            "requested_by": "Faturamento (NF cancelada no ONET)",
                            "requested_at": "17/09/2026",
            ...
            print(f"  - PO #{po.po_number} ({client_name[:35]}): {old_status} -> CANCELLED")

        db.commit()
        print(f"SUCCESS: All {len(pos)} POs and their items successfully cancelled with audit trail committed!")
